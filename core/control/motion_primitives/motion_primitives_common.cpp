...
namespace motion_primitives_common
{
  ...
  std::int64_t compute_forward_progress_um(const local_positioning::snapshot &start_pose, const local_positioning::snapshot &current_pose)
  {
    const std::int64_t dx_um = current_pose.x_um - start_pose.x_um;
    const std::int64_t dy_um = current_pose.y_um - start_pose.y_um;
    const double heading_rad = static_cast<double>(start_pose.heading_urad) / 1000000.0;
    const double forward_progress_um = static_cast<double>(dx_um) * std::cos(heading_rad) + static_cast<double>(dy_um) * std::sin(heading_rad);
    return static_cast<std::int64_t>(std::llround(forward_progress_um));
  }

  std::int64_t compute_remaining_forward_distance_um(const local_positioning::snapshot &start_pose, const local_positioning::snapshot &current_pose, std::int64_t target_distance_um)
  {
    const std::int64_t forward_progress_um = compute_forward_progress_um(start_pose, current_pose);
    return target_distance_um - forward_progress_um;
  }
  ...
  {
    const std::int64_t remaining_distance_um = compute_remaining_forward_distance_um(start_pose, current_pose, target_distance_um);

    if (target_distance_um >= 0)
    {
      return remaining_distance_um <= 0;
    }
  ...
  void begin_settling(state &primitive_state, const local_positioning::snapshot &current_pose, std::uint32_t now_ms)
  {
    ...
    primitive_state.snapshot.state = motion_primitives::execution_state::settling;
    primitive_state.snapshot.status_time_ms = now_ms;
  ...
  void finish(state &primitive_state, bool success, bool timed_out, motion_primitives::error_code failure_code, std::uint32_t now_ms)
  {
    ...
    primitive_state.snapshot.state = motion_primitives::execution_state::complete;
    primitive_state.snapshot.failure_code = failure_code;
  ...
  void begin_active_phase(state &primitive_state, const local_positioning::snapshot &current_pose)
  {
    ...
    primitive_state.snapshot.state = motion_primitives::execution_state::active;
    primitive_state.snapshot.phase_start_pose = current_pose;
  }
}
