...
namespace
{
  void read_current_wheel_samples_from_encoder_api(encoder_input_storage::wheel_sample_array &out_wheels)
  {
    ...
    for (std::size_t i = 0u; i < encoder_input_storage::k_wheel_count; ++i)
    {
      ...
      out_wheels[i].sample_id = sample.sample_id;
      out_wheels[i].status = sample.status;
      out_wheels[i].time_ms = sample.time_ms;
    }
  }

  bool has_any_fresh_wheel_sample(const encoder_input_storage::encoder_snapshot &state, const encoder_input_storage::wheel_sample_array &current_wheels)
  {
    ...
    for (std::size_t i = 0u; i < encoder_input_storage::k_wheel_count; ++i)
    {
      if (current_wheels[i].sample_id != state.current_wheels[i].sample_id)
      {
        return true;
      }
    }
...
namespace encoder_input_storage
{
  ...
  bool sample_from_encoder_api(encoder_snapshot &state, std::uint32_t tick_id)
  {
    ...
    if (!has_any_fresh_wheel_sample(state, current_wheels))
    {
      return false;
    }
