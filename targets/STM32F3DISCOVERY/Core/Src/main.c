...
void SystemClock_Config(void)
{
  ...
  */
  ...
  PeriphClkInit.PeriphClockSelection = RCC_PERIPHCLK_USB|RCC_PERIPHCLK_I2C1|RCC_PERIPHCLK_USART2
...
static void MX_USART2_UART_Init(void)
{
  ...
  // 8N1, oversampling by 16, no flow control.
  USART2->CR1 = 0U;
  USART2->CR2 = 0U;
  USART2->CR3 = USART_CR3_EIE;

  const uint32_t baud_rate = 96000U;
  const uint32_t pclk1_hz = HAL_RCC_GetPCLK1Freq();
  USART2->BRR = (pclk1_hz + (baud_rate / 2U)) / baud_rate;

  // Enable RX interrupt so UART ingress is drained into transport ring-buffer
  // even when foreground code is busy.
  USART2->CR1 = USART_CR1_TE | USART_CR1_RE | USART_CR1_RXNEIE | USART_CR1_UE;

  // Clear stale error flags before enabling IRQ.
